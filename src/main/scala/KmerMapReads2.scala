...
import sext._
import tools.{DNASeq, Kmer, Kmer2, Utils, cKmer}
...
object KmerMapReads2 extends App with LazyLogging {
  ...
  def run(config: Config, sc: SparkContext): Unit = {
    ...
    //2、存储reads的RDD,格式:(1,seq)---make_reads_rdd函数：将原文件格式转换成上诉的格式
    ...
    //3、rdds:同一个seq中所有kmer组成的一个数组和kmer数组对应的reads
