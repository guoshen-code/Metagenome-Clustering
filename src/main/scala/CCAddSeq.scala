...
object CCAddSeq extends App with LazyLogging {
  ...
  def run(config: Config, sc: SparkContext): Unit = {
    ...
    //LPA第一行;919790,2095002,2117976,3011266,3366166.......
    //经过CCAdd处理后的结果:(919790,0)(2095002,0)(2117976,0)(3011266,0)(3366166,0)

    val seqFiles = Utils.get_files(config.reads_input.trim(), config.pattern.trim())
    logger.debug(seqFiles)
    val readsRDD = KmerMapReads2.make_read_id_rdd(seqFiles, config.format, sc).map(x => (x._1.toInt, x._2)) //取带有数字id文件的seqId和seq
